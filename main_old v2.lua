...
local function drawConfiguracion()
    ...
    for i, th in ipairs(themes) do
        local tx = ox + left_w + 24 + ((i - 1) % 2) * (tw + 8)
        local ty = oy + 100 + math.floor((i - 1) / 2) * 66
...
local function drawSoporte()
    ...
    local guide = {
        ...
        {"📝  Registro",     string.format("Registrar nuevos clientes. Planes: Diario ($%d), Semanal ($%d), Mensual ($%d). Guardar crea la transacción automáticamente.", PLAN_PRICES.Diario or 0, PLAN_PRICES.Semanal or 0, PLAN_PRICES.Mensual or 0)},
        ...
        {"⚙  Configuración", "Cambia precios, tema visual y notificaciones sin tocar el código."},
...
function love.mousepressed(x, y, btn)
    ...
    elseif G.screen == "configuracion" then
        ...
        local tw = (right_w - 24 - 8) / 2
        for i, th in ipairs(themes) do
            local tx = ox2 + left_w + 24 + ((i - 1) % 2) * (tw + 8)
            local ty = oy2 + 100 + math.floor((i - 1) / 2) * 66
