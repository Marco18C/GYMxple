...
local function drawRenewDialog()
    ...
    local bh = 34
    local gap = 12
    local bx0 = dx + (dw - (bw * 3 + gap * 2)) / 2
    local by = dy + 104
    for i, p in ipairs(PLAN_OPTIONS) do
        local bx = bx0 + (i - 1) * (bw + gap)
        local active = action.plan == p
        rr(bx, by, bw, bh, 6, active and C.sidebar_sel or C.card2)
        setColor(C.white)
        love.graphics.setFont(G.fonts.small)
    ...
    local c_hov = hover(dx + 86, dy + dh - 50, 120, 34)
    local r_hov = hover(dx + dw - 206, dy + dh - 50, 120, 34)
    drawButton(dx + 86, dy + dh - 50, 120, 34, "Cancelar", C.btn_cancel, C.white, G.fonts.normal, 6, c_hov)
    drawButton(dx + dw - 206, dy + dh - 50, 120, 34, "Renovar", C.btn_green, C.white, G.fonts.normal, 6, r_hov)
end
...
local function applyRenewModal()
    ...
    client.expiry = subscriptionExpiry(client.plan, st)
    saveClients()
    saveTx(PLAN_PRICES[client.plan] or 0, client.plan, (client.nombres or "") .. " " .. (client.apellidos or ""), "renovacion")
...
local function drawEditClientDialog()
    ...
    love.graphics.setFont(G.fonts.small)
    love.graphics.print("Inicio de la suscripción", fx, fy + 188)
    setColor(C.gray)
    local start_ts = tsFromDateParts(action.cal_year, action.cal_month, action.selected_day or 1)
    love.graphics.printf(dateStr(os.date("*t", start_ts)), fx, fy + 206, fw, "left")
...
local function saveEditClientModal()
    ...
    client.expiry = subscriptionExpiry(client.plan, client.start_ts)
    saveClients()
...
function love.keypressed(key)
    ...
    elseif ctrl and key == "m" then
        ...
        editorState("new_note_hour").cursor = 1
        editorState("new_note_hour").anchor = 1
        return
...
function love.mousepressed(x, y, btn)
    ...
    if G.renew_modal then
        ...
        if client then
            ...
            for i, p in ipairs(PLAN_OPTIONS) do
                if hover(bx0 + (i - 1) * (bw + gap), by, bw, 34) then
        ...
        if hover(dx + 86, dy + dh - 50, 120, 34) then
            G.renew_modal = nil
            return
        end
        if hover(dx + dw - 206, dy + dh - 50, 120, 34) then
            applyRenewModal()
            return
        end
        return
